...
namespace MondayOFF {
    internal sealed class Interstitial : FullscreenAdType {
        ...
        private float _lastInterstitialTimestamp = default;
        ...
        internal Interstitial(AdSettings setting, float interval) {
            Debug.Log("[EVERYDAY] Createing Interstitial Ad");

            _adUnitID = setting.interstitialAdUnitId;
            _interval = interval;
            _lastInterstitialTimestamp = -interval;
            ...
            if (setting.resetTimerOnRewarded) {
                MaxSdkCallbacks.Rewarded.OnAdHiddenEvent += ResetTimer;
            }
